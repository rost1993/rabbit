...
main(int argc, char *argv[])
{
	...
	ECRYPT_keysetup(&ctx, (uint8_t *)key, 16, 64);
	ECRYPT_ivsetup(&ctx, iv);

	while((byte = fread(buf, 1, block, fp)) > 0) {
		if(action == 1)
			ECRYPT_process_bytes(0, &ctx, buf, out, byte);
			//ECRYPT_process_packet(0, &ctx, iv, buf, out, byte);
		else
			ECRYPT_process_bytes(1, &ctx, buf, out, byte);
			//ECRYPT_process_packet(1, &ctx, iv, buf, out, byte);
